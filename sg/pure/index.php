...
<body id="produk">
						...
						<div class="col">
							<div class="detail-spek">
								...
								<h4 class="text-capitalize mb-3 opacity-75">
									<?php echo $data['description SG'] ?>
								</h4>
								<p class="text-capitalize mb-3 opacity-75">
									<?php echo $data['ingredients SG'] ?>
								</p>
								<p class="text-capitalize mb-5 opacity-75">
									net. wight<br><?php echo $data['weight'] ?> gram
								</p>
		...
		<section class="related-produk text-center" style="background-color: #252525; padding-bottom: 4rem;">
			<div class="container">
				...
				<div id="carousel1" class="carousel slide produk-slider" data-bs-ride="true">
					<div class="carousel-inner">
						<?php
						$query = mysqli_query($koneksi, "SELECT * FROM produk WHERE collection ='pure' ");
						while($data = mysqli_fetch_array($query)){
							?>
							<div class="carousel-item">
								<div class="col">
									<div class="card card-produk">
										<a href="<?php echo $ucafeSG ?><?php echo $data['collection']; ?>/?idProduct=<?php echo $data['id'];?>" class="d-inline">
											<div class="card-img">
						...
						<?php } ?>
						<?php
						/*$query = mysqli_query($koneksi, "SELECT * FROM produk WHERE collection != 'pure' ORDER BY RAND() LIMIT 5 ");*/
						$query = mysqli_query($koneksi, "SELECT * FROM produk WHERE collection != 'pure' LIMIT 5 ");
						while($data = mysqli_fetch_array($query)){
							?>
							<div class="carousel-item">
								<div class="col">
									<div class="card card-produk">
										<a href="<?php echo $ucafeSG ?><?php echo $data['collection']; ?>/?idProduct=<?php echo $data['id'];?>" class="d-inline mb-3">
											<div class="card-img">
												<img src="<?php echo $ucafe ?><?php echo $data['image 1']; ?>" class="img-fluid">
											</div>
										</a>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$('footer .btn-products .btn-pure').addClass('active');
		});
